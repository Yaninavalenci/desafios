...
-- Creé el usuario con el nombre 'usuario_lectura'
CREATE USER 'usuario_lectura'@'localhost' IDENTIFIED BY 'only_read';

-- Dí permisos de solo lectura a todas las tablas de la base de datos.
GRANT SELECT ON mitaller.* TO 'usuario_lectura'@'localhost';

SHOW GRANTS FOR usuario_lectura@localhost;

-- Creé el usuario con el nombre 'usuario_modificación'
CREATE USER 'usuario_modificacion'@'localhost' IDENTIFIED BY 'manage';

-- Dí permisos de lectura, inserción y modificación a todas las tablas de la base de datos.
GRANT SELECT, INSERT, UPDATE ON mitaller.* TO 'usuario_modificacion'@'localhost';

SHOW GRANTS FOR usuario_modificacion@localhost;

-- Generé una transacción que garantiza que se realice una inserción de cliente solo si no hay un cliente existente con la misma información y también maneja posibles errores dentro de una transacción.

DELIMITER $$

CREATE PROCEDURE sp_transaccion_con_condicional(
    IN p_telefono INT,
    IN p_dni INT,
...
BEGIN
    ...
    FROM clientes
    WHERE telefono = p_telefono OR dni = p_dni;

    IF cliente_existente > 0 THEN
        -- El cliente ya existe
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Error: El cliente ya existe';
    ELSE
        -- El cliente no existe, realiza la inserción.
        BEGIN
            -- Inicia una nueva transacción para la inserción.
            START TRANSACTION;
            
            -- Intenta la inserción
            
            INSERT INTO clientes (telefono, dni, nombre, apellido)
            VALUES (p_telefono, p_dni, p_nombre, p_apellido);
        END;
    END IF;
END $$
...
CALL sp_transaccion_con_condicional(1127712261, 32518231, 'David', 'Gilmour');

-- Generé otra transacción que crea una orden de trabajo, insertando el cliente si no existe (si existe, lo valida y no hace la inserción, pero si crea la orden).
--  Condicionalmente si el técnico tiene 3 o más ordenes de trabajo, no crea la órden y devuelve el mensaje correspondiente.

DELIMITER $$

CREATE PROCEDURE sp_transaccion_orden_con_log(
    IN p_telefono_cliente INT,
    IN p_dni_cliente INT,
    IN p_nombre_cliente VARCHAR(45),
    IN p_apellido_cliente VARCHAR(45),
    IN p_id_tecnico INT,
    IN p_nuevo_item_ordentrabajo VARCHAR(45),
    IN p_fecha_inicio_ordentrabajo DATE,
    IN p_fecha_fin_ordentrabajo DATE
)
BEGIN
    ...
    FROM clientes
    WHERE telefono = p_telefono_cliente OR dni = p_dni_cliente
    LIMIT 1;

    IF cliente_id IS NULL THEN
        -- No hay cliente existente, realiza varias operaciones dentro de la transacción
        -- Inserta un nuevo cliente
        INSERT INTO clientes (telefono, dni, nombre, apellido)
        VALUES (p_telefono_cliente, p_dni_cliente, p_nombre_cliente, p_apellido_cliente);

        SET cliente_id = LAST_INSERT_ID();
    END IF;

    -- Verifica la cantidad de órdenes en curso para el técnico
    SET @cantidad_ordenes_en_curso = 0;
    SELECT COUNT(*) INTO @cantidad_ordenes_en_curso
    FROM ordenes_de_trabajo
    WHERE id_tecnico = p_id_tecnico AND id_estado = 5; -- El estado "en curso" tiene ID 5

    IF @cantidad_ordenes_en_curso < 3 THEN
        -- Inserta una nueva orden de trabajo
        INSERT INTO ordenes_de_trabajo (id_cliente, id_usuario, id_estado, id_tecnico, item, fecha_inicio, fecha_fin)
        VALUES (cliente_id, 1, 5, p_id_tecnico, p_nuevo_item_ordentrabajo, p_fecha_inicio_ordentrabajo, p_fecha_fin_ordentrabajo);

        -- Si todo está bien, hace commit
        COMMIT;
    ELSE
        -- Limite de órdenes en curso para el técnico
        SET rollback_required = TRUE;
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Advertencia: El técnico ya tiene tres órdenes en curso. La orden de trabajo no fue creada.';
    END IF;
...
CALL sp_transaccion_orden_con_log(1122614000, 32987654, 'Hilda', 'Lizarazu', 2, 'depiladora', '23-10-11', null);

Select * from ordenes_de_trabajo;
Select * from clientes;

-- Agregué adicionalmente un procedure para ver cuantas ordenes tiene un tecnico salvo en estados finalizado y facturado.

DELIMITER //

CREATE PROCEDURE obtener_ordenes_pendientes(IN tecnico_id INT)
BEGIN
    SELECT
        ot.id_tecnico AS id,
    ...
    WHERE
        t.id_tecnico = tecnico_id
        AND e.tipo_estado NOT IN ('finalizado', 'Facturado');
END //
...
CALL obtener_ordenes_pendientes(2);
